...
extern size_t max_swap_offset;     //max_swap_offset = ide_device_size(SWAP_DEV_NO) / (PGSIZE / SECTSIZE); 

/* *
 * swap_offset - takes a swap_entry (saved in pte), and returns       //没看懂
 * the corresponding offset in swap mem_map. //把一个swap_entry  取前24bit（后续乘8找扇区号）
 * */
...
int swap_init(void);                    //做check_swap();
