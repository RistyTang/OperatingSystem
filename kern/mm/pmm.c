...
extern pde_t __boot_pgdir;       //entry.S
 ...
 * vpd is set bellow.
 * */
pte_t * const vpt = (pte_t *)VPT;    //页目录表中第一个目录表项指向的页表的起始虚地址:FAC00000   (VPT处)
pde_t * const vpd = (pde_t *)PGADDR(PDX(VPT), PDX(VPT), 0);   //页目录表的起始虚地址:FAFEB000
...
//取消了最后的两个函数kmalloc(size_t n)、kfree(void *ptr, size_t n) 